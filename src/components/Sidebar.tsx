  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
        {tabs.map((item) => {
          const IconComponent =
            IconMap[item.iconName || "BarChart3"] || BarChart3;
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5 flex-1 min-w-0">
                <IconComponent
                  className={`w-[18px] h-[18px] shrink-0 ${isActive ? "text-brand-primary" : "text-brand-on-surface-variant opacity-70"}`}
                />
                {isEditing ? (
                  <input
                    ref={editInputRef}
                  ...
                  <span className="hidden lg:block text-[13px] truncate">
                    {item.sidebarLabel}
                  </span>
                )}
              </div>
        ...
        >
          <ClipboardList
            className={`w-[18px] h-[18px] shrink-0 ${currentTab === "board_menu" ? "text-brand-primary" : "text-brand-on-surface-variant opacity-70"}`}
          />
          <span className="hidden lg:block text-[13px] truncate">
            게시판 (요청)
          </span>
      ...
      <div className="px-4 py-4 mt-auto border-t border-brand-outline-variant bg-brand-surface-lowest space-y-2">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="hidden lg:block overflow-hidden flex-1">
            <span className="block text-xs font-medium text-brand-on-surface truncate">
              Manager
            </span>
            <span className="block text-[10px] text-brand-on-surface-variant truncate opacity-70">
              Administrator
            </span>
          </div>
        </div>

        {/* Settings Tab (Fixed to Footer) */}
        <button
        ...
        {/* Native Data Actions - Always visible, but handles fallback if clicked on web */}
        <div className="pt-2 border-t border-brand-outline-variant space-y-2">
          <button
            onClick={() => {
              // @ts-ignore
              if (window.__TAURI_INTERNALS__ || window.__TAURI_IPC__ || window.__TAURI__) {
                if (onTauriSave) onTauriSave();
              } else {
                alert("이 기능은 데스크톱 앱(Tauri) 환경에서만 작동합니다. 웹 브라우저에서는 사용할 수 없습니다.");
              }
            }}
        ...
        >
          ...
          <span className="hidden lg:inline">
            {isExportingHtml ? "내보내는 중..." : "HTML 내보내기"}
          </span>
        </button>
      </div>
    </aside>
