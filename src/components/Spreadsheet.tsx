...
import { Requirement, Priority, Status, Assignee, Column, ColumnType } from '../types';
...
export default function Spreadsheet({ activeTabId, requirements, setRequirements: originalSetRequirements, assigneesPool, setAssigneesPool, columns, setColumns, openComingSoonModal, socket, dbPath, currentUser, activeLocks = {}, tabs = [], tabDataMap = {}, dashboardFilter, onDashboardFilterConsumed }: SpreadsheetProps) {
  ...
  const setRequirements = useCallback((action: React.SetStateAction<Requirement[]>) => {
  ...
  // --- END UNDO / REDO STATE ---
  // Queries & Filters
  const [tabFilters, setTabFilters] = useState<Record<string, {
    searchTerm: string;
  ...
  const setSearchTerm = (val: string) => setTabFilters(prev => ({ ...prev, [activeTabId || 'global']: { ...currentTabFilters, ...(prev[activeTabId || 'global'] || {}), searchTerm: val } }));
  const setPriorityFilter = (val: Priority | 'ALL') => setTabFilters(prev => ({ ...prev, [activeTabId || 'global']: { ...currentTabFilters, ...(prev[activeTabId || 'global'] || {}), priorityFilter: val } }));
  const setStatusFilter = (val: Status | 'ALL') => setTabFilters(prev => ({ ...prev, [activeTabId || 'global']: { ...currentTabFilters, ...(prev[activeTabId || 'global'] || {}), statusFilter: val } }));
  const setSortField = (val: 'id' | 'title' | 'dueDate' | 'status' | null) => setTabFilters(prev => ({ ...prev, [activeTabId || 'global']: { ...currentTabFilters, ...(prev[activeTabId || 'global'] || {}), sortField: val } }));
  const setSortDirection = (val: 'asc' | 'desc') => setTabFilters(prev => ({ ...prev, [activeTabId || 'global']: { ...currentTabFilters, ...(prev[activeTabId || 'global'] || {}), sortDirection: val } }));

  // Custom Columns List
  const [showAddColumnModal, setShowAddColumnModal] = useState(false);
  const [showFormulaHelp, setShowFormulaHelp] = useState(false);
  const [editingColumnDefId, setEditingColumnDefId] = useState<string | null>(null);
  const [columnToDelete, setColumnToDelete] = useState<string | null>(null);
  const [newColumnName, setNewColumnName] = useState('');
  const [newColumnType, setNewColumnType] = useState<ColumnType>('text');
  const [editingColumnNameId, setEditingColumnNameId] = useState<string | null>(null);
  ...
  const [currencyExchangeRatesInput, setCurrencyExchangeRatesInput] = useState({ KRW: 1, USD: 1400, EUR: 1500, GBP: 1750 });
  const [currencyDecimalPlacesInput, setCurrencyDecimalPlacesInput] = useState<number>(0);
  const [decimalPlacesInput, setDecimalPlacesInput] = useState<string>('');
  const [rollupRelIdInput, setRollupRelIdInput] = useState('');
  const [rollupAggTypeInput, setRollupAggTypeInput] = useState<'count' | 'percent_done' | 'sum' | 'avg'>('count');
  ...
  // Inflation PV state
  const [inflationAmountColIdInput, setInflationAmountColIdInput] = useState("");
  const [inflationTitleColIdInput, setInflationTitleColIdInput] = useState("title");
  const [inflationRefTabIdInput, setInflationRefTabIdInput] = useState("");
  const [inflationMatchColIdInput, setInflationMatchColIdInput] = useState("id");
  const [inflationRefMatchColIdInput, setInflationRefMatchColIdInput] = useState("id");
  const [inflationBaseDateColIdInput, setInflationBaseDateColIdInput] = useState("");
  const [inflationTargetDateColIdInput, setInflationTargetDateColIdInput] = useState("");
  const [inflationRatesInput, setInflationRatesInput] = useState<{word: string, rate: number, baseShip?: string, note?: string}[]>([]);
  const [isInflationRatesExpanded, setIsInflationRatesExpanded] = useState(true);
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`app_grid_col_widths_${activeTabId || 'global'}`);
  ...
  const setColumnWidths = useCallback((updater: React.SetStateAction<Record<string, number>>) => {
    _setColumnWidths(prev => {
      ...
      localStorage.setItem(`app_grid_col_widths_${activeTabId || 'global'}`, JSON.stringify(next));
      return next;
    });
  }, [activeTabId]);
  ...
  const [assigneeManagerPos, setAssigneeManagerPos] = useState<{ x: number, y: number } | undefined>(undefined);
  ...
  const [minimizedColumns, _setMinimizedColumns] = useState<Record<string, boolean>>({});
  
  useEffect(() => {
    setIsSingleLineView(localStorage.getItem(`singleLineView_${activeTabId || 'global'}`) === 'true');
    const saved = localStorage.getItem(`minimizedCols_${activeTabId || 'global'}`);
    if (saved) {
      try { _setMinimizedColumns(JSON.parse(saved)); } catch(e) { _setMinimizedColumns({}); }
    } else {
  ...
  const toggleSingleLineView = () => {
    setIsSingleLineView(prev => {
      const next = !prev;
      localStorage.setItem(`singleLineView_${activeTabId || 'global'}`, String(next));
      return next;
    });
  };

  const setMinimizedColumns = useCallback((updater: React.SetStateAction<Record<string, boolean>>) => {
    _setMinimizedColumns(prev => {
      const next = typeof updater === 'function' ? updater(prev) : updater;
      localStorage.setItem(`minimizedCols_${activeTabId || 'global'}`, JSON.stringify(next));
      return next;
    });
  }, [activeTabId]);

  // Column level text search filters (Per tab)
  const [tabColumnSearchTerms, setTabColumnSearchTerms] = useState<Record<string, Record<string, string>>>({});
  const columnSearchTerms = tabColumnSearchTerms[activeTabId || 'global'] || {};
  const setColumnSearchTerms = useCallback((updater: React.SetStateAction<Record<string, string>>) => {
  ...
  const [showFilterColumnId, setShowFilterColumnId] = useState<string | null>(null);
  const [filterPopupCoords, setFilterPopupCoords] = useState<{ top: number; left: number } | null>(null);
  ...
  const [showDescriptionEditColId, setShowDescriptionEditColId] = useState<string | null>(null);
  const [descriptionInput, setDescriptionInput] = useState('');
  const [newColumnOptionsInput, setNewColumnOptionsInput] = useState('');

  // Active Inline Cell Editors
  const [activeCellEditor, setActiveCellEditorState] = useState<{ rowId: string; field: string } | null>(null);
  ...
  const setActiveCellEditor = useCallback(async (val: { rowId: string; field: string } | null) => {
       ...
       if (window.__TAURI_INTERNALS__ && dbPath && currentUser) {
          try {
             ...
             await invokeCall('release_item_lock', { 
                projectPath: dbPath, itemId: `${activeTabId}:${activeCell.rowId}`, userId: currentUser.id 
             });
          } catch(e) {}
       }
       ...
       if (window.__TAURI_INTERNALS__ && dbPath && currentUser) {
          try {
             ...
             await invokeCall('acquire_item_lock', { 
                projectPath: dbPath, itemId: `${activeTabId}:${val.rowId}`, userId: currentUser.id, userName: currentUser.name 
             });
          } catch (e: any) {
             // Revert on failure
             setActiveCellEditorState(null);
  ...
  // Dropdown States
  const [showPriorityDropdownId, setShowPriorityDropdownId] = useState<string | null>(null);
  ...
  const [showAssigneeDropdownId, setShowAssigneeDropdownId] = useState<string | null>(null);
  const [assigneeDropdownPos, setAssigneeDropdownPos] = useState({ top: 0, left: 0 });
  const [showSelectDropdown, setShowSelectDropdown] = useState<{rowId: string, colId: string} | null>(null);
  const [selectDropdownPos, setSelectDropdownPos] = useState({ top: 0, left: 0 });
  ...
  useEffect(() => {
    ...
    } else if (dashboardFilter.type === 'custom_column' && dashboardFilter.columnId && dashboardFilter.value) {
      // Add isolated filter for the clicked custom column value
      setColumnSearchTerms({ [dashboardFilter.columnId]: dashboardFilter.value });
    }
  ...
  // Filter & Search Logic
  const filteredAndSortedRequirements = useMemo(() => requirements
    .filter(req => {
      // Priority filter
      ...
      if (statusFilter !== 'ALL') {
        ...
        const normalizedStatus = (s === 'TODO' || s === '대기중') ? 'TODO' : 
      ...
      for (const colId of Object.keys(columnSearchTerms)) {
        ...
        const colTerm = (isNegation ? rawTerm.slice(1) : rawTerm).toLowerCase();
        if (colTerm === '') continue;
        ...
        if (colId === 'status' && !matchColumn(translateStatus(req.status))) return false;
        if (colId === 'priority' && !matchColumn(req.priority)) return false;
        if (colId === 'dueDate' && !matchColumn(req.dueDate)) return false;
        if (colId === 'assignees') {
          if (isNegation) {
            if (req.assignees.some(a => a.name.toLowerCase().includes(colTerm))) return false;
          } else {
            if (!req.assignees.some(a => a.name.toLowerCase().includes(colTerm))) return false;
          }
        }
  ...
  const updateRequirementField = useCallback((rowId: string, field: string, value: any) => {
  ...
  const handleAddOrEditCustomColumn = () => {
      ...
      setColumns(prev => {
        const newCols = prev.map(c => {
          if (c.id === editingColumnDefId) {
             let options = (newColumnType === 'status' || newColumnType === 'select') ? (newColumnType === 'select' ? newColumnOptionsInput : statusOptionsInput).split(',').map(s => s.trim()).filter(Boolean) : undefined;
             ...
             return {
                ...
                buttonAction: newColumnType === 'button' ? buttonActionInput : undefined,
                buttonLabel: newColumnType === 'button' ? buttonLabelInput : undefined,
                currencyAmountColId: newColumnType === 'currency_usd' ? currencyAmountColIdInput : undefined,
                currencyCodeColId: newColumnType === 'currency_usd' ? currencyCodeColIdInput : undefined,
                currencyExchangeRates: newColumnType === 'currency_usd' ? currencyExchangeRatesInput : undefined,
                currencyDecimalPlaces: newColumnType === 'currency_usd' ? currencyDecimalPlacesInput : undefined,
                rollupRelId: newColumnType === 'rollup' ? rollupRelIdInput : undefined,
                rollupAggType: newColumnType === 'rollup' ? rollupAggTypeInput : undefined,
                lookupTabId: newColumnType === 'lookup' ? lookupTabIdInput : undefined,
                lookupMatchMyColId: newColumnType === 'lookup' ? lookupMatchMyColIdInput : undefined,
                lookupMatchTargetColId: newColumnType === 'lookup' ? lookupMatchTargetColIdInput : undefined,
                lookupReturnTargetColId: newColumnType === 'lookup' ? lookupReturnTargetColIdInput : undefined,
                inflationAmountColId: newColumnType === 'inflation_pv' ? inflationAmountColIdInput : undefined,
                inflationTitleColId: newColumnType === 'inflation_pv' ? inflationTitleColIdInput : undefined,
                inflationRefTabId: newColumnType === 'inflation_pv' ? inflationRefTabIdInput : undefined,
                inflationMatchColId: newColumnType === 'inflation_pv' ? inflationMatchColIdInput : undefined,
                inflationRefMatchColId: newColumnType === 'inflation_pv' ? inflationRefMatchColIdInput : undefined,
                inflationBaseDateColId: newColumnType === 'inflation_pv' ? inflationBaseDateColIdInput : undefined,
                inflationTargetDateColId: newColumnType === 'inflation_pv' ? inflationTargetDateColIdInput : undefined,
                inflationRates: newColumnType === 'inflation_pv' ? inflationRatesInput : undefined,
                decimalPlaces: decimalPlacesInput !== '' ? Number(decimalPlacesInput) : undefined,
             };
          }
          if (newColumnType === 'currency_usd' && c.type === 'currency_usd' && c.id !== editingColumnDefId) {
    ...
    setColumns(prev => {
      ...
      return [...nextCols, { 
        ...
        isCustom: true, 
        type: newColumnType,
        options: (newColumnType === 'status' || newColumnType === 'select') ? (newColumnType === 'select' ? newColumnOptionsInput : statusOptionsInput).split(',').map(s => s.trim()).filter(Boolean) : undefined,
        formula: newColumnType === 'formula' ? formulaInput : undefined,
        buttonAction: newColumnType === 'button' ? buttonActionInput : undefined,
        buttonLabel: newColumnType === 'button' ? buttonLabelInput : undefined,
        currencyAmountColId: newColumnType === 'currency_usd' ? currencyAmountColIdInput : undefined,
      ...
      requirements.forEach((req, i) => {
        ...
        columns.forEach((col, colIdx) => {
          ...
          if (col.id === 'id') val = req.id;
             ...
             const usdValue = resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates);
             const fractionDigits = col.currencyDecimalPlaces !== undefined ? col.currencyDecimalPlaces : 0;
             val = usdValue === '' ? 'N/A' : (isNaN(usdValue) ? 'N/A' : '$' + Number(usdValue).toLocaleString(undefined, { minimumFractionDigits: fractionDigits, maximumFractionDigits: fractionDigits }));
          }
          else if (col.type === 'formula') {
             try {
               val = String(resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates));
               if (!val.startsWith('Error') && val !== '' && !isNaN(Number(val)) && col.decimalPlaces !== undefined) {
                 val = Number(val).toLocaleString(undefined, { minimumFractionDigits: col.decimalPlaces, maximumFractionDigits: col.decimalPlaces });
               }
             } catch (e: any) {
               val = `Error: ${e.message}`;
             }
             ...
             val = resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates);
             if (val && !isNaN(Number(val)) && col.decimalPlaces !== undefined) val = String(Number(val).toFixed(col.decimalPlaces));
          }
          else if (col.type === 'inflation_pv') {
             try {
                const numVal = resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates);
                if (numVal !== '' && !isNaN(Number(numVal))) {
                   const dp = col.decimalPlaces !== undefined ? col.decimalPlaces : 0;
                   val = Number(numVal).toLocaleString(undefined, { minimumFractionDigits: dp, maximumFractionDigits: dp });
                } else {
                   val = numVal;
                }
             } catch(e: any) {
          ...
          if (isRoot) {
            ...
            cell.font = { ...cell.font as any, bold: true, color: { argb: 'FFFFFFFF' } };
          } else {
      ...
      if (isTauri) {
        ...
        try {
          const filePath = await save({
            ...
            defaultPath: `Requirements_Export_${new Date().toISOString().split('T')[0]}.xlsx`
          });
          ...
          if (filePath) {
            ...
            } else if (buffer && typeof (buffer as any).length !== 'undefined') {
        ...
        saveAs(blob, `Requirements_Export_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const [superContextMenuGroup, setSuperContextMenuGroup] = useState<string | null>(null);
  const [contextMenuPos, setContextMenuPos] = useState<{ x: number, y: number }>({ x: 0, y: 0 });
  const [hoveredColumnId, setHoveredColumnId] = useState<string | null>(null);
  const [hoverTitleCoords, setHoverTitleCoords] = useState<{ x: number, y: number } | null>(null);
  const [frozenColumnIds, setFrozenColumnIds] = useState<Record<string, string | null>>({});
  ...
  const setFrozenColumnId = useCallback((id: string | null) => {
    setFrozenColumnIds(prev => ({ ...prev, [activeTabId || 'global']: id }));
  }, [activeTabId]);
  ...
  const handleDuplicateRow = useCallback((sourceRowId: string, insertAfterId?: string) => {
  ...
  // 8. Excel Paste Logic
  const handleGridPaste = useCallback((e: React.ClipboardEvent, startRowId: string, startColId: string) => {
    ...
    if (pasteData.length === 0 || (pasteData.length === 1 && pasteData[0].length <= 1)) return; // Fallback to normal if just text
            ...
            currentReq.dueDate = cleanedText || currentReq.dueDate;
             ...
             for (const name of names) {
                ...
                if (foundPool) {
                  ...
                  const newAssignee = { id: newAssigneeId, name: name, avatarUrl: '' };
  ...
  const getStickyStyle = useCallback((isHeader = false): React.CSSProperties => {
    if (isHeader) {
      return { 
  ...
  const getCellStickyStyle = useCallback((colId: string | 'index' | 'checkbox', isHeader = false): React.CSSProperties => {
    let base = getStickyStyle(isHeader);

    if (frozenOffsets) {
      const isFrozen = colId === 'index' || colId === 'checkbox' || frozenOffsets.offsets[colId as string] !== undefined;
      if (isFrozen) {
        const left = colId === 'index' ? 0 : colId === 'checkbox' ? 40 : frozenOffsets.offsets[colId as string];
        ...
        base = { 
          ...
          backgroundColor: isHeader ? 'var(--color-brand-surface-highest)' : 'var(--row-bg)' 
        };
  ...
  return (
    <div className="bg-brand-surface border border-brand-outline rounded-[2rem] shadow-xl overflow-hidden flex flex-col flex-1 min-h-[500px] animate-fade-slide-up delay-100">
      ...
      <div ref={parentRef} onScroll={handleGridScroll} className="overflow-x-auto overflow-y-auto overscroll-y-contain flex-1 min-h-0 w-full relative">
                    ...
                    if (colSpan > 0) {
                      ths.push(
                         <th
                          ...
                          colSpan={colSpan}
                          className={`py-1 text-center ${currentGroup ? "border-x border-t-2 border-b-0 border-brand-primary/60 bg-brand-surface-high/80 text-brand-primary font-bold shadow-sm cursor-context-menu" : "border-r border-brand-outline-variant"}`}
                      ...
                      <div
                        ...
                        onDoubleClick={(e) => {
                          ...
                          filteredAndSortedRequirements.forEach(req => {
                            let text = '';
                            if (col.isCustom || !['id', 'title', 'priority', 'dueDate', 'status'].includes(col.id)) {
                               try {
                                  text = String(resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates) || '');
                               } catch (e) {}
                            } else {
                               text = String(req[col.id as keyof Requirement] || '');
                            }
          ...
          <tbody className="text-sm text-brand-on-surface divide-y divide-brand-outline-variant/60 leading-[1.35]">
            {filteredAndSortedRequirements.length === 0 ? (
              ...
              (() => {
                ...
                return (
                  <>
                    {paddingTop > 0 && <tr><td style={{ height: `${paddingTop}px`, padding: 0, border: 'none' }} colSpan={columns.length + 3} /></tr>}
                    {virtualRows.map((virtualRow) => {
                      const req = filteredAndSortedRequirements[virtualRow.index];
                      if (!req) return null;
                      return (
                        <SpreadsheetRow
                          key={req.id}
                          ...
                          isLockedByOther={
                            ...
                            activeLocks[`${activeTabId}:${req.id}`].userId !== currentUser?.id
                          }
                          lockedByName={activeLocks[`${activeTabId}:${req.id}`]?.userName ?? null}
                          ...
                          statusRef={statusRef}
                          showSelectDropdown={showSelectDropdown?.rowId === req.id ? showSelectDropdown.colId : null}
                    ...
                    {paddingBottom > 0 && <tr><td style={{ height: `${paddingBottom}px`, padding: 0, border: 'none' }} colSpan={columns.length + 3} /></tr>}
                  </>
                );
              })()
            )}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="inflation_pv">물가상승 현가화 (Inflation PV)</option>
            ...
            {newColumnType === "inflation_pv" && (
              <div className="space-y-3">
                <p className="text-[10px] text-brand-outline-variant leading-relaxed">
                  금액을 물가상승률을 적용하여 현가화합니다. 참조 탭에서 견적일과 발주일을 가져와 개월 수 차이를 계산하고, 항목의 특정 단어별 상승률을 반영합니다.
                </p>
                <div>
                  <label className="block text-xs text-brand-on-surface-variant font-medium mb-1.5">대상 금액 열 (현재 탭)</label>
                  <select value={inflationAmountColIdInput} onChange={(e) => setInflationAmountColIdInput(e.target.value)} className="w-full px-3 py-2 bg-brand-surface text-sm border border-brand-outline-variant rounded-lg focus:ring-1 focus:ring-brand-primary focus:outline-none">
                    <option value="">열 선택</option>
                    {columns.filter(c => c.type === 'number' || c.type === 'currency_usd' || c.type === 'formula' || c.type === 'text' || c.type === 'lookup').map(c => <option value={c.id} key={c.id}>{c.label}</option>)}
                  </select>
                </div>

                
                <div className="p-3 bg-brand-surface-high border border-brand-outline-variant rounded-lg space-y-3">
                  <h4 className="text-xs font-semibold text-brand-on-surface">참조 탭(Dates Tab) 설정 영역</h4>
                  <p className="text-[10px] text-brand-on-surface-variant">
                    다른 탭에서 날짜(견적일, 발주일)를 가져오기 위한 설정입니다. VLOOKUP처럼 두 탭을 연결할 매치 키를 지정하세요.
                  </p>
                  <div>
                    <label className="block text-xs text-brand-on-surface-variant font-medium mb-1.5">참조할 탭 선택</label>
                    <select value={inflationRefTabIdInput} onChange={(e) => setInflationRefTabIdInput(e.target.value)} className="w-full px-3 py-2 bg-brand-surface text-sm border border-brand-outline-variant rounded-lg focus:ring-1 focus:ring-brand-primary focus:outline-none">
                      <option value="">탭 선택</option>
                      {tabs?.map((t: any) => <option value={t.id} key={t.id}>{t.sidebarLabel || t.dashboardTitle || t.id}</option>)}
                    </select>
                  </div>
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <label className="block text-[11px] text-brand-on-surface-variant font-medium mb-1.5">현재 탭 매치 키</label>
                      <select value={inflationMatchColIdInput} onChange={(e) => setInflationMatchColIdInput(e.target.value)} className="w-full px-2 py-1.5 bg-brand-surface text-xs border border-brand-outline-variant rounded focus:ring-1 focus:ring-brand-primary focus:outline-none">
                        <option value="id">ID</option>
                        <option value="title">Title</option>
                        {columns.map(c => <option value={c.id} key={c.id}>{c.label}</option>)}
                      </select>
                    </div>
                    <div className="flex-1">
                      <label className="block text-[11px] text-brand-on-surface-variant font-medium mb-1.5">참조 탭 매치 키</label>
                      <select value={inflationRefMatchColIdInput} onChange={(e) => setInflationRefMatchColIdInput(e.target.value)} disabled={!inflationRefTabIdInput} className="w-full px-2 py-1.5 bg-brand-surface text-xs border border-brand-outline-variant rounded focus:ring-1 focus:ring-brand-primary focus:outline-none disabled:opacity-50">
                        <option value="id">ID</option>
                        <option value="title">Title</option>
                        {inflationRefTabIdInput && tabDataMap?.[inflationRefTabIdInput]?.columns?.map((c: any) => <option value={c.id} key={c.id}>{c.label}</option>)}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 pt-1 border-t border-brand-outline-variant/50">
                    <div className="flex-1">
                      <label className="block text-[11px] text-brand-on-surface-variant font-medium mb-1.5">가져올 견적일(기준일)</label>
                      <select value={inflationBaseDateColIdInput} onChange={(e) => setInflationBaseDateColIdInput(e.target.value)} disabled={!inflationRefTabIdInput} className="w-full px-2 py-1.5 bg-brand-surface text-xs border border-brand-outline-variant rounded focus:ring-1 focus:ring-brand-primary focus:outline-none disabled:opacity-50">
                    ...
                    <div className="flex-1">
                      <label className="block text-[11px] text-brand-on-surface-variant font-medium mb-1.5">가져올 발주일(대상일)</label>
                      <select value={inflationTargetDateColIdInput} onChange={(e) => setInflationTargetDateColIdInput(e.target.value)} disabled={!inflationRefTabIdInput} className="w-full px-2 py-1.5 bg-brand-surface text-xs border border-brand-outline-variant rounded focus:ring-1 focus:ring-brand-primary focus:outline-none disabled:opacity-50">
                        <option value="">선택</option>
                        <option value="dueDate">Due Date</option>
                        {inflationRefTabIdInput && tabDataMap?.[inflationRefTabIdInput]?.columns?.map((c: any) => <option value={c.id} key={c.id}>{c.label}</option>)}
                      </select>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    <label className="block text-xs text-brand-on-surface-variant font-medium">호선별 월간 물가상승률(%)</label>
                    <button 
                      onClick={() => setIsInflationRatesExpanded(!isInflationRatesExpanded)}
                  ...
                  {isInflationRatesExpanded ? (
                    <div className="border border-brand-outline-variant rounded-lg overflow-hidden bg-brand-surface text-xs">
                      <div className="grid grid-cols-[2fr_1.5fr_2fr_2fr_30px] gap-1 bg-brand-surface-high p-1.5 border-b border-brand-outline-variant font-semibold text-brand-on-surface text-[11px]">
                        ...
                        <div className="px-1 truncate text-center">INF/년(%)</div>
                        ...
                        onPaste={(e) => {
                          ...
                          for (const row of rows) {
                            ...
                            if (cols.length >= 2) {
                              ...
                              if (rateStr.includes('%')) rateStr = rateStr.replace('%', '');
                              let rate = parseFloat(rateStr.replace(/[^0-9.-]+/g, ''));
                          ...
                          if (newRates.length > 0) {
                            e.preventDefault();
                            setInflationRatesInput(prev => [...prev, ...newRates]);
                          }
                        }}
                      >
                        {inflationRatesInput.length === 0 && (
                          <div className="text-center py-4 text-[10px] text-brand-outline-variant italic">
                            엑셀에서 복사(Ctrl+C) 후 여기에 붙여넣기(Ctrl+V) 하세요.
                          </div>
                        )}
                        {inflationRatesInput.map((rateObj, idx) => (
                          <div key={idx} className="grid grid-cols-[2fr_1.5fr_2fr_2fr_30px] gap-1 items-center">
                            <input 
                              type="text" placeholder="검색 단어" value={rateObj.word} 
                              onChange={e => {
                            ...
                            />
                            <input 
                              type="number" placeholder="0.0" value={rateObj.rate} 
                              onChange={e => {
                                const newRates = [...inflationRatesInput];
                            ...
                            />
                            <input 
                              type="text" placeholder="호선명" value={rateObj.baseShip || ''} 
                              onChange={e => {
                                const newRates = [...inflationRatesInput];
                            ...
                            />
                            <input 
                              type="text" placeholder="비고" value={rateObj.note || ''} 
                              onChange={e => {
                                const newRates = [...inflationRatesInput];
                            ...
                            <button onClick={() => {
                              ...
                              setInflationRatesInput(newRates);
                            }} className="text-red-500 hover:bg-red-500/10 rounded w-full h-full flex items-center justify-center transition-colors">✕</button>
                          </div>
                        ))}
                      </div>
                      <div className="p-1.5 border-t border-brand-outline-variant bg-brand-surface-high">
                        <button onClick={() => setInflationRatesInput([...inflationRatesInput, {word: '', rate: 0, baseShip: '', note: ''}])} className="text-[11px] px-2 py-1 bg-brand-surface border border-brand-outline-variant rounded hover:bg-brand-surface-highest transition-colors cursor-pointer w-full">
                          + 항목 추가
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="p-2 border border-brand-outline-variant rounded-lg bg-brand-surface text-[10px] text-brand-on-surface-variant flex justify-between items-center">
                      <span>총 {inflationRatesInput.length}개의 호선별 물가상승률 설정됨</span>
                      <button onClick={() => setIsInflationRatesExpanded(true)} className="px-2 py-1 bg-brand-surface-high border border-brand-outline-variant rounded hover:bg-brand-surface-highest transition-colors cursor-pointer">펼치기</button>
                    </div>
                  )}
                </div>
              </div>
               ...
               <div>
                ...
                {(() => {
                  ...
                  try {
                    ...
                    const dummyReq = requirements[0] || { id: 'dummy', title: '', customColumns: {} };
                    const boundSUMIFS = (...args: any[]) => SUMIFS(tabDataMap, tabs, args[0], args[1], args[2], args[3]);
                    const today = new Date().toISOString().split('T')[0];
                    const GET_VAL = (id: string) => resolveColumnValue(dummyReq, id, columns, tabDataMap, tabs, exchangeRates);
                    testFn(dummyReq, today, SUM, AVERAGE, IF, DAYS, MONTHS, exchangeRates.KRW, exchangeRates.USD, exchangeRates.EUR, boundSUMIFS, GET_VAL);
                    return <p className="text-emerald-500 text-[10px] mt-1">✓ 수식이 유효합니다.</p>;
                  } catch (err: any) {
                    return <p className="text-red-500 text-[10px] mt-1 font-medium bg-red-500/10 p-1.5 rounded border border-red-500/20">🚨 수식 오류: {err.message}</p>;
                  }
                })()}
            ...
            {newColumnType === "currency_usd" && (
               <div>
                ...
                <p className="text-[10px] text-brand-outline-variant mb-2">환율을 직접 입력하여 사용할 수 있습니다.</p>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] text-brand-outline-variant mb-1">KRW</label>
                    <input
                      type="number"
                      value={currencyExchangeRatesInput.KRW}
                      onChange={(e) => setCurrencyExchangeRatesInput({...currencyExchangeRatesInput, KRW: Number(e.target.value)})}
                  ...
                  <div>
                    <label className="block text-[10px] text-brand-outline-variant mb-1">USD (원/달러)</label>
                    <input
                      type="number"
                      value={currencyExchangeRatesInput.USD}
                      onChange={(e) => setCurrencyExchangeRatesInput({...currencyExchangeRatesInput, USD: Number(e.target.value)})}
                      className="w-full bg-brand-surface border border-brand-outline-variant rounded px-2 py-1 text-xs text-brand-on-surface focus:outline-none focus:ring-1 focus:ring-brand-primary [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] text-brand-outline-variant mb-1">EUR (원/유로)</label>
                    <input
                      type="number"
                      value={currencyExchangeRatesInput.EUR}
                      onChange={(e) => setCurrencyExchangeRatesInput({...currencyExchangeRatesInput, EUR: Number(e.target.value)})}
                      className="w-full bg-brand-surface border border-brand-outline-variant rounded px-2 py-1 text-xs text-brand-on-surface focus:outline-none focus:ring-1 focus:ring-brand-primary [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] text-brand-outline-variant mb-1">GBP (원/파운드)</label>
                    <input
                      type="number"
                      value={currencyExchangeRatesInput.GBP}
                      onChange={(e) => setCurrencyExchangeRatesInput({...currencyExchangeRatesInput, GBP: Number(e.target.value)})}
                      className="w-full bg-brand-surface border border-brand-outline-variant rounded px-2 py-1 text-xs text-brand-on-surface focus:outline-none focus:ring-1 focus:ring-brand-primary [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                  </div>
                </div>
            ...
            {newColumnType === "rollup" && (
               <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => setRollupAggTypeInput(e.target.value as 'count' | 'percent_done' | 'sum' | 'avg')}
                  className="w-full px-3 py-2 bg-brand-surface text-sm border border-brand-outline-variant rounded-lg focus:ring-1 focus:ring-brand-primary focus:outline-none cursor-pointer"
                >
        ...
        >
          <div className="space-y-4">
            ...
            <div className="max-h-60 overflow-y-auto space-y-1.5 border border-brand-outline-variant rounded-lg p-2 bg-brand-surface-low">
              ...
              {assigneesPool.map((a) => (
                 <div key={a.id} className="flex items-center justify-between p-2 hover:bg-brand-surface hover:shadow-sm rounded transition-colors group border border-transparent hover:border-brand-outline-variant bg-brand-surface-lowest">
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            if (editingAssigneeName.trim()) {
                              setAssigneesPool((prev) => prev.map(x => x.id === a.id ? { ...x, name: editingAssigneeName.trim() } : x));
                              setRequirements((prev) => prev.map((req) => ({
                                ...req,
                                assignees: req.assignees.map(x => x.id === a.id ? { ...x, name: editingAssigneeName.trim() } : x)
                              })));
                            }
...
이 담당자가 배정된 모든 요구사항에서도 담당자가 일괄 삭제됩니다.`)
      ...
      {contextMenuColId &&
        createPortal(
          ...
          {selectedColumnIds.length > 1 && selectedColumnIds.includes(contextMenuColId) && (
            <button
              className="w-full px-4 py-2 text-left hover:bg-brand-surface flex items-center gap-2 cursor-pointer transition-colors"
              onClick={() => {
          ...
          {columns.find(c => c.id === contextMenuColId)?.groupName && (
            <button
              className="w-full px-4 py-2 text-left hover:bg-brand-surface flex items-center gap-2 cursor-pointer transition-colors"
              onClick={() => {
                setColumns(prev => prev.map(c => c.id === contextMenuColId ? { ...c, groupName: undefined } : c));
                setContextMenuColId(null);
              }}
            >
          ...
          <button
            className="w-full px-4 py-2 text-left hover:bg-brand-surface flex items-center gap-2 cursor-pointer transition-colors"
            onClick={() => {
              setColumns(prev => prev.map(c => c.id === contextMenuColId ? { ...c, truncateText: !c.truncateText } : c));
              setContextMenuColId(null);
            }}
          >
            <AlignLeft className="w-3.5 h-3.5 text-brand-outline-variant" />
            {columns.find(c => c.id === contextMenuColId)?.truncateText ? "내용 펼치기" : "내용 감추기 (2줄 고정)"}
          </button>
          ...
          <div className="px-4 py-2 hover:bg-brand-surface flex flex-col gap-1 transition-colors">
            ...
            <div className="flex gap-1.5 mt-1">
              {['transparent', '#1e293b', '#1e1b4b', '#064e3b', '#4c0519', '#3b0764', '#422006'].map(color => (
                <button
                  key={color}
                  ...
                  onClick={() => {
                    setColumns(prev => prev.map(c => c.id === contextMenuColId ? { ...c, backgroundColor: color === 'transparent' ? undefined : color } : c));
                  }}
                />
              ))}
            </div>
          ...
          <button
            className="w-full px-4 py-2 text-left hover:bg-brand-surface flex items-center gap-2 cursor-pointer transition-colors"
            onClick={() => {
              const val = prompt(`'${columns.find(c => c.id === contextMenuColId)?.label}' 열에 일괄 입력할 값을 입력하세요:`);
              if (val !== null) {
                setRequirements(prev => prev.map(req => {
                   const colId = contextMenuColId;
                   if (!colId) return req;
                   if (['id', 'title', 'priority', 'dueDate', 'status'].includes(colId)) {
                      return { ...req, [colId]: val };
                   }
                   return { ...req, customColumns: { ...(req.customColumns || {}), [colId]: val } };
                }));
              }
              setContextMenuColId(null);
            }}
          ...
          {contextMenuColId === 'status' && (
            <div className="px-4 py-2 hover:bg-brand-surface flex flex-col gap-1 transition-colors group relative cursor-pointer text-brand-primary">
              ...
              <div className="absolute left-full top-0 ml-1 hidden group-hover:flex flex-col bg-brand-surface border border-brand-outline-variant rounded-lg shadow-xl w-32 z-50">
                <button className="px-3 py-2 text-left hover:bg-brand-surface-high text-brand-on-surface text-xs font-medium" onClick={() => { setRequirements(prev => prev.map(req => ({ ...req, status: 'TODO' }))); setContextMenuColId(null); }}>대기중</button>
                <button className="px-3 py-2 text-left hover:bg-brand-surface-high text-brand-on-surface text-xs font-medium" onClick={() => { setRequirements(prev => prev.map(req => ({ ...req, status: 'IN_PROGRESS' }))); setContextMenuColId(null); }}>검토중</button>
                <button className="px-3 py-2 text-left hover:bg-brand-surface-high text-brand-on-surface text-xs font-medium" onClick={() => { setRequirements(prev => prev.map(req => ({ ...req, status: 'DONE' }))); setContextMenuColId(null); }}>검토완료</button>
              </div>
            </div>
          )}
          ...
          <button
            ...
            onClick={() => {
              ...
              if (col) {
                ...
                setCurrencyExchangeRatesInput((col.currencyExchangeRates as any) || { KRW: 1, USD: 1400, EUR: 1500, GBP: 1750 });
                ...
                setInflationTitleColIdInput(col.inflationTitleColId || 'title');
                setInflationRefTabIdInput(col.inflationRefTabId || '');
                setInflationMatchColIdInput(col.inflationMatchColId || 'id');
                setInflationRefMatchColIdInput(col.inflationRefMatchColId || 'id');
                setInflationBaseDateColIdInput(col.inflationBaseDateColId || '');
                setInflationTargetDateColIdInput(col.inflationTargetDateColId || '');
                setInflationRatesInput(col.inflationRates || []);
                setDecimalPlacesInput(col.decimalPlaces !== undefined ? String(col.decimalPlaces) : '');
              }
        ...
        createPortal(
          ...
          >
            <button
              className="w-full px-4 py-2 text-left hover:bg-brand-surface flex items-center gap-2 cursor-pointer transition-colors"
              onClick={() => {
                setColumns(prev => prev.map(c => c.groupName === superContextMenuGroup ? { ...c, groupName: undefined } : c));
                setSuperContextMenuGroup(null);
              }}
            >
              <Grid className="w-3.5 h-3.5 text-brand-outline-variant" />
...
const SUMIFS = (tabDataMap: Record<string, any>, tabs: any[], targetTab: string, sumCol: string, critCol: string, critVal: any) => {
  if (!tabDataMap || !tabs) return 0;
  const tab = tabs.find((t: any) => t.sidebarLabel === targetTab || t.id === targetTab || t.dashboardTitle === targetTab);
  if (!tab) return 0;
  const data = tabDataMap[tab.id];
  if (!data || !data.requirements || !data.columns) return 0;
  const sCol = data.columns.find((c: any) => c.label === sumCol || c.id === sumCol);
  const cCol = data.columns.find((c: any) => c.label === critCol || c.id === critCol);
  if (!sCol || !cCol) return 0;
  let sum = 0;
  data.requirements.forEach((r: any) => {
    let sVal = ['title', 'dueDate', 'createdAt', 'status', 'priority'].includes(sCol.id) ? (r as any)[sCol.id] : r.customColumns?.[sCol.id];
    let cVal = ['title', 'dueDate', 'createdAt', 'status', 'priority'].includes(cCol.id) ? (r as any)[cCol.id] : r.customColumns?.[cCol.id];
...
export const resolveColumnValue = (req: any, colId: string, columns: any[], tabDataMap: Record<string, any>, tabs: any[], exchangeRates: any, callStack = new Set<string>()): any => {
  ...
  try {
    ...
    if (col.type === 'formula') {
      ...
      const boundSUMIFS = (...args: any[]) => SUMIFS(tabDataMap, tabs, args[0], args[1], args[2], args[3]);
      const GET_VAL = (id: string) => resolveColumnValue(req, id, columns, tabDataMap, tabs, exchangeRates, new Set(callStack));
      return func(req, today, SUM, AVERAGE, IF, DAYS, MONTHS, exchangeRates.KRW, exchangeRates.USD, exchangeRates.EUR, boundSUMIFS, GET_VAL);
    }

    if (col.type === 'inflation_pv') {
       const amountRaw = col.inflationAmountColId === 'title' ? req.title : resolveColumnValue(req, col.inflationAmountColId, columns, tabDataMap, tabs, exchangeRates, new Set(callStack));
       ...
       if (col.inflationMatchColId) {
          myVal = col.inflationMatchColId === 'id' ? req.id : (col.inflationMatchColId === 'title' ? req.title : String(resolveColumnValue(req, col.inflationMatchColId, columns, tabDataMap, tabs, exchangeRates, new Set(callStack)) || ''));
       }
       ...
       if (targetTab && col.inflationRefMatchColId && myVal) {
          const matchedReq = targetTab?.requirements?.find((r: any) => {
             const tVal = col.inflationRefMatchColId === 'id' ? r.id : (col.inflationRefMatchColId === 'title' ? r.title : String(r.customColumns?.[col.inflationRefMatchColId!] || ''));
             return tVal === myVal;
          });
          if (matchedReq) {
             baseDateStr = col.inflationBaseDateColId === 'dueDate' ? matchedReq.dueDate : matchedReq.customColumns?.[col.inflationBaseDateColId!] || '';
             targetDateStr = col.inflationTargetDateColId === 'dueDate' ? matchedReq.dueDate : matchedReq.customColumns?.[col.inflationTargetDateColId!] || '';
          }
       }
       ...
       if (matchedRate && baseDateStr && targetDateStr) {
          ...
          const annualMultiplier = matchedRate < 2 ? matchedRate : (1 + matchedRate / 100);
    ...
    if (col.type === 'currency_usd') {
        ...
        if (amountCol && currencyCol) {
           const rawAmountStr = String(resolveColumnValue(req, amountCol.id, columns, tabDataMap, tabs, exchangeRates, new Set(callStack))).replace(/[^0-9.-]+/g, "");
           const amount = Number(rawAmountStr) || 0;
           const curr = String(resolveColumnValue(req, currencyCol.id, columns, tabDataMap, tabs, exchangeRates, new Set(callStack))).toUpperCase();
           
           const rates = col.currencyExchangeRates || { KRW: 1, USD: 1400, EUR: 1500, GBP: 1750 };
           let krwValue = amount;
    ...
    if (col.type === 'rollup') {
        ...
        if (!col.rollupMatchMyColId || !col.rollupMatchTargetColId || !col.rollupTargetValueColId || !col.rollupAggType) return '';
        const myVal = resolveColumnValue(req, col.rollupMatchMyColId, columns, tabDataMap, tabs, exchangeRates, new Set(callStack));
        const matchedReqs = rReqs.filter((r: any) => {
            const tVal = col.rollupMatchTargetColId === 'id' ? r.id : (col.rollupMatchTargetColId === 'title' ? r.title : r.customColumns?.[col.rollupMatchTargetColId!]);
            return tVal === myVal;
        });
        const vals = matchedReqs.map((r: any) => col.rollupTargetValueColId === 'id' ? r.id : (col.rollupTargetValueColId === 'title' ? r.title : r.customColumns?.[col.rollupTargetValueColId!]));
        const numVals = vals.map((v: any) => Number(String(v).replace(/[^0-9.-]+/g,""))).filter((n: any) => !isNaN(n));
        
        if (col.rollupAggType === 'sum') return numVals.reduce((a: any, b: any) => a + b, 0);
        if (col.rollupAggType === 'avg') return numVals.length ? numVals.reduce((a: any, b: any) => a + b, 0) / numVals.length : 0;
        if (col.rollupAggType === 'max') return numVals.length ? Math.max(...numVals) : 0;
        if (col.rollupAggType === 'min') return numVals.length ? Math.min(...numVals) : 0;
    ...
    if (col.type === 'lookup') {
        ...
        if (!col.lookupMatchMyColId || !col.lookupMatchTargetColId || !col.lookupReturnTargetColId) return '';
        const myVal = resolveColumnValue(req, col.lookupMatchMyColId, columns, tabDataMap, tabs, exchangeRates, new Set(callStack));
        const matchedReq = lReqs.find((r: any) => {
            const tVal = col.lookupMatchTargetColId === 'id' ? r.id : (col.lookupMatchTargetColId === 'title' ? r.title : r.customColumns?.[col.lookupMatchTargetColId!]);
            return tVal === myVal;
        });
        if (matchedReq) {
            return col.lookupReturnTargetColId === 'id' ? matchedReq.id : (col.lookupReturnTargetColId === 'title' ? matchedReq.title : matchedReq.customColumns?.[col.lookupReturnTargetColId!]);
        }
        return '';
    }
...
const getFormulaFn = (f: string, columns: any[]) => {
  ...
  if (!formulaCache.has(cacheKey)) {
    try {
      ...
      // Add generic aliases
      const aliases = {
        '제목': 'title', 'title': 'title', '요구사항명칭': 'title', 'req.description': 'title',
        '마감일': 'dueDate', 'duedate': 'dueDate', '기한': 'dueDate',
        '상태': 'status', 'status': 'status',
        '우선순위': 'priority', 'priority': 'priority',
        '담당자': 'assignees', 'assignees': 'assignees'
      };
      ...
      let parsed = f.replace(/\[([\s\S]+?)\]/g, (match, p1) => {
        ...
        const matchedId = labelToIdMap.get(normLabel) || labelToIdMap.get(normLabelLower);
        if (matchedId) {
          if (['title', 'dueDate', 'createdAt', 'status', 'priority'].includes(matchedId)) {
      ...
      formulaCache.set(cacheKey, new Function('req', 'today', 'SUM', 'AVERAGE', 'IF', 'DAYS', 'MONTHS', 'KRW', 'USD', 'EUR', 'SUMIFS', 'GET_VAL', `try { return ${parsed || '""'}; } catch(e) { console.error("Formula Eval Error in getFormulaFn:", e, "Original:", \`${f.replace(/`/g, '\\`')}\`, "Parsed:", \`${parsed.replace(/`/g, '\\`')}\`); throw e; }`));
    } catch (err: any) {
      console.error("Formula Parsing Error:", err, f);
      formulaCache.set(cacheKey, () => { throw err; });
    }
  }
  return formulaCache.get(cacheKey);
};

const SpreadsheetRow = React.memo(React.forwardRef(function SpreadsheetRow({
  req,
  isSelected,
  isSingleLineView,
  dragOverRowId,
  ...
  dataIndex
                ...
                const normalizedStatusForDone = ((req.status as string) === 'TODO' || (req.status as string) === '대기중' || !req.status) ? 'TODO' :
                                                ((req.status as string) === 'IN_PROGRESS' || (req.status as string) === '검토중') ? 'IN_PROGRESS' :
                                                ((req.status as string) === 'DONE' || (req.status as string) === '검토완료') ? 'DONE' : 'TODO';
                ...
                else if (normalizedStatusForDone === 'IN_PROGRESS') rowTextColorClass = 'text-green-700 font-medium';
                ...
                return (
                  ...
                  >
                    ...
                    <td style={getCellStickyStyle('index')} className={`${isSingleLineView ? 'px-2 py-0.5' : 'p-2'} border-r border-brand-outline-variant text-center w-10 text-brand-on-surface-variant/40 align-middle`}>
                      <div className={`flex ${isSingleLineView ? 'flex-row' : 'flex-col'} flex-wrap justify-center items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity`}>
                        <button 
                          onClick={(e) => { e.stopPropagation(); handleDuplicateRow(req.id, req.id); }}
                    ...
                    {/* Checkbox cell */}
                    <td style={getCellStickyStyle('checkbox')} className={`${isSingleLineView ? 'px-2 py-1' : 'p-3'} border-r border-brand-outline-variant text-center select-none align-middle`}>
                      <div 
                      ...
                      >
                        {isSelected && <Check className="w-3 h-3 text-brand-on-surface" strokeWidth={4} />}
                      </div>
                    </td>
                    ...
                    {columns.map(col => {
                      ...
                      const bgStyle = col.backgroundColor ? { backgroundImage: `linear-gradient(to bottom right, ${col.backgroundColor}40, transparent)` } : {};
                      const cellStyle: React.CSSProperties = { width, minWidth: width, maxWidth: width, ...baseStickyStyle, ...textAlignStyle, ...bgStyle };
                      const shadowClass = '';
                      const truncateClass = isSingleLineView ? 'whitespace-nowrap overflow-hidden text-ellipsis' : (col.truncateText ? 'line-clamp-2 overflow-hidden overflow-ellipsis break-words whitespace-normal' : 'whitespace-pre-wrap break-words');
                      ...
                      if (isMinimized) {
                        return (
                          <td 
                            ...
                            title="열 펼치기 (클릭)"
                          >
                            <span className="text-brand-outline-variant select-none tracking-widest text-[10px] font-bold mx-auto flex justify-center w-full">...</span>
                          </td>
                        );
                      }
                      ...
                      if (col.id === 'id') {
                        ...
                        return (
                          <td 
                            ...
                            className={`px-2 py-1 border-r border-brand-outline-variant font-mono text-[13px] text-brand-on-surface-variant font-medium ${isLockedByOther ? 'cursor-not-allowed bg-brand-surface-high/30' : 'cursor-text hover:bg-brand-primary/5'} transition-colors whitespace-normal break-words align-top relative ${shadowClass}`}
                            title={isLockedByOther ? `${lockedByName} 님이 편집 중` : undefined}
                          >
                            {isLockedByOther && (
                              <div className="absolute top-1 right-1 opacity-70" title={`${lockedByName} 님이 편집 중`}>
                      ...
                      if (col.id === 'title') {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`font-medium min-h-[1.2rem] ${isEditing ? 'opacity-0' : ''}`}>
                      ...
                      if (col.id === 'priority') {
                        ...
                        return (
                          <td key={col.id} style={cellStyle} className={`px-2 py-1 border-r border-brand-outline-variant relative align-top ${shadowClass} ${isDone ? 'opacity-60' : ''}`}>
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                if (isPriorityOpen) {
                                  ...
                                  const rect = e.currentTarget.getBoundingClientRect();
                                  setPriorityDropdownPos({ top: rect.bottom, left: rect.left });
                            ...
                            >
                              <span className={`px-2 py-0.5 rounded ${badgeColors[req.priority] || badgeColors.MEDIUM}`}>
                            ...
                            {isPriorityOpen && createPortal(
                              <div 
                                ref={priorityRef}
                                style={{ position: 'fixed', top: priorityDropdownPos.top + 4, left: priorityDropdownPos.left }}
                              ...
                              >
                                {(['HIGH', 'MEDIUM', 'LOW'] as Priority[]).map(p => (
                                  ...
                                  >
                                    <span>{labels[p]}</span>
                                    {req.priority === p && <Check className="w-3 h-3 text-brand-primary" />}
                                  </button>
                                ))}
                              </div>, document.body)}
                          </td>
                      ...
                      if (col.id === 'assignees') {
                        return (
                          <td key={col.id} style={cellStyle} className={`px-2 py-1 border-r border-brand-outline-variant relative align-top ${isSingleLineView ? 'whitespace-nowrap overflow-hidden' : 'whitespace-normal'} ${shadowClass}`}>
                            <div 
                              onClick={(e) => {
                                if (isAssigneeOpen) {
                                  setShowAssigneeDropdownId(null);
                                  ...
                                  const rect = e.currentTarget.getBoundingClientRect();
                                  setAssigneeDropdownPos({ top: rect.bottom, left: rect.left });
                            ...
                            >
                              {req.assignees.length === 0 ? (
                                <div className="text-[11px] text-brand-outline-variant italic">담당자 없음</div>
                              ) : (
                                req.assignees.map((a, idx) => (
                                  <div 
                                    ...
                                    title={a.name}
                                  >
                                    <span className={isSingleLineView ? 'truncate max-w-[40px]' : ''}>{a.name}</span>
                                  </div>
                                ))
                              )}
                            </div>
                            ...
                            {isAssigneeOpen && createPortal(
                              <div 
                                ref={assigneeRef}
                                style={{ position: 'fixed', top: assigneeDropdownPos.top + 4, left: assigneeDropdownPos.left }}
                                  ...
                                  {assigneesPool.map(member => {
                                    ...
                                    return (
                                      ...
                                      >
                                          ...
                                          >
                                            {isAssigned && <Check className="w-2.5 h-2.5 text-brand-on-surface" strokeWidth={4} />}
                                          </div>
                      ...
                      if (col.id === 'status') {
                        ...
                        const labels: any = { TODO: '대기중', IN_PROGRESS: '검토중', DONE: '검토완료' };
                        ...
                        return (
                          <td key={col.id} style={cellStyle} className={`px-2 py-1 border-r border-brand-outline-variant relative align-top ${shadowClass}`}>
                            <button 
                              onClick={(e) => {
                                  e.stopPropagation();
                                  if (isStatusOpen) {
                                    ...
                                    const rect = e.currentTarget.getBoundingClientRect();
                                    setStatusDropdownPos({ top: rect.bottom, left: rect.left });
                                    setShowStatusDropdownId(req.id);
                                  }
                              }}
                              className="cursor-pointer transition-transform hover:scale-105"
                            >
                              <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-medium leading-none ${statusColors[normalizedStatus] || statusColors.TODO}`}>
                            ...
                            {isStatusOpen && createPortal(
                              <div 
                                ref={statusRef}
                                style={{ position: 'fixed', top: statusDropdownPos.top + 4, left: statusDropdownPos.left }}
                              ...
                              >
                                {(['TODO', 'IN_PROGRESS', 'DONE'] as Status[]).map(st => (
                                  ...
                                  >
                                    <span>{labels[st]}</span>
                                    {normalizedStatus === st && <Check className="w-3 h-3 text-brand-primary" />}
                                  </button>
                                ))}
                              </div>, document.body)}
                          </td>
                      ...
                      if (col.isCustom) {
                        ...
                        if (col.type === 'checkbox') {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                {isChecked && <Check className="w-3 h-3 text-brand-on-surface" strokeWidth={4} />}
                              </div>
                            </td>
                          );
                        }
                        ...
                        if (col.type === 'currency_usd') {
                          let result = resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates);
                          if (result === '') {
                             ...
                             const fractionDigits = col.currencyDecimalPlaces !== undefined ? col.currencyDecimalPlaces : 0;
                             result = '$' + Number(result).toLocaleString(undefined, { minimumFractionDigits: fractionDigits, maximumFractionDigits: fractionDigits });
                          }
                          return (
                            <td 
                              key={col.id} 
                        ...
                        if (col.type === 'inflation_pv') {
                          ...
                          if (!result.startsWith('Error') && result !== '' && !isNaN(Number(result))) {
                            const dp = col.decimalPlaces !== undefined ? col.decimalPlaces : 0;
                            formattedResult = Number(result).toLocaleString(undefined, { minimumFractionDigits: dp, maximumFractionDigits: dp });
                          }
                          ...
                          return (
                            <td 
                              ...
                              className={`px-2 py-1 border-r border-brand-outline-variant align-top text-brand-primary font-bold bg-brand-surface-lowest/50 ${shadowClass}`}
                            >
                              <div className={`flex items-center gap-1.5 justify-end ${truncateClass}`}>
                        ...
                        if (col.type === 'lookup') {
                          let result = String(resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates));
                          if (result && !isNaN(Number(result)) && col.decimalPlaces !== undefined) result = String(Number(result).toFixed(col.decimalPlaces));
                          ...
                          return (
                            ...
                            >
                              <div className={`font-medium min-h-[1.2rem] ${truncateClass}`}>
                        ...
                        if (col.type === 'formula') {
                          let result = '';
                          try {
                            result = String(resolveColumnValue(req, col.id, columns, tabDataMap, tabs, exchangeRates));
                          } catch (e: any) {
                            result = `Error: ${e.message}`;
                          }

                          if (!result.startsWith('Error') && result !== '' && !isNaN(Number(result)) && col.decimalPlaces !== undefined) {
                        ...
                        if (col.type === 'relation') {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className={`flex ${isSingleLineView ? 'flex-row overflow-hidden whitespace-nowrap h-[1.8rem] items-center' : 'flex-wrap'} gap-1 ${isEditing ? 'opacity-0' : ''}`}>
                                {relIds.length === 0 && <span className="opacity-35 italic font-light">-</span>}
                                {relIds.map(rid => {
                                  const linkedReq = reqById.get(rid);
                                  if (!linkedReq) return <span key={rid} className="text-brand-error text-[10px] border border-brand-error/20 bg-brand-error/10 px-1 rounded">{rid} 없음</span>;
                                  return (
                                    <span key={rid} className="text-[10px] font-medium bg-brand-tertiary/10 text-brand-tertiary border border-brand-tertiary/30 px-1 py-0.5 rounded flex items-center gap-1 leading-none shadow-sm">
                        ...
                        if (col.type === 'rollup') {
                           ...
                           const linkedReqs = relIds.map(rid => reqById.get(rid)).filter(Boolean) as Requirement[];
                           ...
                           if (resultStr !== '-' && !resultStr.includes('%') && !isNaN(Number(resultStr)) && col.decimalPlaces !== undefined) {
                            ...
                            >
                              ...
                              >
                                ...
                                <optgroup label="할 일 (TODO)">
                                  {options.filter((_, i) => i < options.length / 3).map(opt => (
                                    <option key={opt} value={opt}>{opt}</option>
                                  ))}
                                </optgroup>
                                <optgroup label="진행 중 (IN PROGRESS)">
                                  {options.filter((_, i) => i >= options.length / 3 && i < (options.length / 3) * 2).map(opt => (
                                    <option key={opt} value={opt}>{opt}</option>
                                  ))}
                                </optgroup>
                                <optgroup label="완료 (DONE)">
                                  {options.filter((_, i) => i >= (options.length / 3) * 2).map(opt => (
                                    <option key={opt} value={opt}>{opt}</option>
                                  ))}
                                </optgroup>
                              </select>
                        ...
                        if (col.type === 'select') {
                          const options = col.options || [];
                          const selectedValues = typeof cellVal === 'string' ? cellVal.split(',').map(s => s.trim()).filter(Boolean) : [];
                            ...
                            >
                              <div 
                                onClick={(e) => {
                                  if (isSelectOpen) {
                                    ...
                                    const rect = e.currentTarget.getBoundingClientRect();
                                    setSelectDropdownPos({ top: rect.bottom, left: rect.left });
                              ...
                              >
                                {selectedValues.length === 0 ? (
                                  <div className="text-[11px] text-brand-outline-variant italic">선택 없음</div>
                                ) : (
                                  selectedValues.map((val, idx) => (
                                    <div 
                                      ...
                                      title={val}
                                    >
                                      <span className={isSingleLineView ? 'truncate max-w-[40px]' : ''}>{val}</span>
                                    </div>
                                  ))
                                )}
                              </div>
                              ...
                              {isSelectOpen && createPortal(
                                <div 
                                  ...
                                  style={{ position: 'fixed', top: selectDropdownPos.top + 4, left: selectDropdownPos.left }}
                                  className="w-52 bg-brand-surface-high border border-brand-outline-variant rounded-xl shadow-xl p-3 z-[9999] animate-fade-slide-up text-xs font-semibold"
                                >
                                  <p className="text-[11px] text-brand-outline mb-2 pb-1 border-b border-brand-outline-variant">
                                          ...
                                          <input 
                                            ...
                                            onChange={(e) => {
                                              if (e.target.checked) {
                                                ...
                                                updateRequirementField(req.id, col.id, newVals.join(', '));
                                              } else {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`font-medium min-h-[1.2rem] ${truncateClass} ${isEditing ? 'opacity-0' : ''}`}>
                              {inputType === 'number' && col.decimalPlaces !== undefined && cellVal && !isNaN(Number(cellVal)) 
