  ...
  useEffect(() => {
    const fetchLogs = async () => {
      ...
      try {
        ...
        const dailyLogs: ChangelogEntry[] = await invoke("read_changelog", {
          projectPath: dbPath,
          dateStr,
        });
      ...
      <div className="bg-brand-surface border border-brand-outline-variant rounded-xl shadow-2xl max-w-2xl w-full h-[80vh] flex flex-col animate-fade-slide-up">
        <div className="flex items-center justify-between px-6 py-4 border-b border-brand-outline-variant bg-brand-surface-lowest rounded-t-xl">
          <div className="flex items-center gap-2 text-brand-on-surface">
            ...
            <h2 className="font-bold font-title-md">
              작업 이력 (실시간 포렌식)
            </h2>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-brand-surface-high rounded-lg transition-colors text-brand-outline-variant hover:text-brand-on-surface cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-brand-surface space-y-3">
          {loading ? (
            <p className="text-center text-sm text-brand-outline-variant p-4">
              분석 중...
            </p>
          ) : logs.length === 0 ? (
            <p className="text-center text-sm text-brand-outline-variant p-4">
            ...
            logs.map((log, i) => (
              <div
                key={i}
                className="flex gap-4 p-3 bg-brand-surface-lowest border border-brand-outline-variant rounded-lg"
              >
