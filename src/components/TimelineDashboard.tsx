...
const BACKGROUND_EVENTS = [
  { label: '코로나19', start: new Date(2020, 0, 1).getTime(), end: new Date(2023, 11, 31).getTime(), color: 'rgba(239, 68, 68, 0.15)', borderColor: 'rgba(239, 68, 68, 0.3)' },
  { label: '우크라이나 전쟁', start: new Date(2022, 1, 24).getTime(), end: Date.now(), color: 'rgba(251, 191, 36, 0.15)', borderColor: 'rgba(251, 191, 36, 0.3)' },
  { label: '이란 전쟁', start: new Date(2026, 0, 1).getTime(), end: Date.now(), color: 'rgba(168, 85, 247, 0.15)', borderColor: 'rgba(168, 85, 247, 0.3)' },
];
...
export default function TimelineDashboard({ requirements, columns, config, onConfigChange, onDashboardFilter }: TimelineDashboardProps) {
  ...
  const defaultKlCol = dateColumns.find(c => c.id === 'kl_date')?.id || dateColumns[0]?.id || '';
  const defaultBaseCol = dateColumns.find(c => c.id === 'quote_date')?.id || dateColumns[0]?.id || '';
  const defaultTitleCol = textColumns.find(c => c.id === 'title')?.id || textColumns[0]?.id || '';
  const defaultCheckboxCol = checkboxColumns.find(c => c.id === 'show_in_timeline')?.id || '';

  const currentConfig: TimelineConfig = config || {
    targetColId: defaultKlCol,
    baseColId: defaultBaseCol,
    titleColId: defaultTitleCol,
    checkboxColId: defaultCheckboxCol,
  ...
  const { targetColId, baseColId, titleColId, checkboxColId, manualStartDate, manualEndDate, showBaseDate, showBackgroundEvents, diffUnit, extraLabelColId } = currentConfig;

  // Data mapping
  const timelineItems = useMemo(() => {
    let filtered = requirements;
    if (checkboxColId) {
      filtered = filtered.filter(req => req.customColumns?.[checkboxColId] === 'true');
    }
    ...
    const items = filtered.map(req => {
      ...
      const title = titleColId === 'title' ? req.title : (req.customColumns?.[titleColId] || 'No Title');
      const extraLabel = extraLabelColId ? (extraLabelColId === 'title' ? req.title : (req.customColumns?.[extraLabelColId] || '')) : '';
      ...
      if (targetTime && baseTime) {
        diffMonths = Math.round((targetTime - baseTime) / (1000 * 60 * 60 * 24 * 30.436875));
      }
    ...
    return items;
  }, [requirements, checkboxColId, targetColId, baseColId, titleColId, extraLabelColId]);
  ...
  const uniqueBaseDates = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([time, data]) => ({ time, dateStr: data.dateStr }));
  }, [timelineItems]);
  ...
  const bounds = useMemo(() => {
    ...
    let minTime = timelineItems.length > 0 ? Math.min(...timelineItems.map(i => i.targetTime as number)) : Date.now();
    
    // The base reference end: Base Quote Date + 5 years
    const allBaseDates = requirements
      .filter(r => !checkboxColId || r.customColumns?.[checkboxColId] === 'true')
      .map(r => r.customColumns?.[baseColId])
      .filter(d => !!d)
      .map(d => new Date(d as string).getTime());

    let maxBaseTime = allBaseDates.length > 0 ? Math.max(...allBaseDates) : Date.now();
    ...
    return { start, end };
  }, [timelineItems, requirements, baseColId, checkboxColId, manualStartDate, manualEndDate]);
  ...
  const handleItemClick = (reqTitle: string) => {
    if (onDashboardFilter) {
      onDashboardFilter({ type: 'custom_column', columnId: 'title', value: reqTitle });
    }
  };

  const totalRange = bounds.end - bounds.start;
  const getLeftPct = (time: number) => ((time - bounds.start) / totalRange) * 100;
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/5 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-slate-100">타임라인 대시보드</h2>
          <span className="text-xs text-slate-500 ml-2">(우클릭하여 설정 변경)</span>
        </div>
        <button 
      ...
      <div className="flex-1 py-6 px-8 relative w-full flex items-center justify-center min-h-[220px]">
        {timelineItems.length === 0 ? (
          <div className="text-slate-500 text-sm italic">선택된 데이터가 없습니다. 스프레드시트에서 체크박스를 활성화해주세요.</div>
        ) : (
          <div className="relative w-full flex items-center justify-center">
            ...
            <div className="absolute left-0 right-0 h-2 rounded-full overflow-hidden flex bg-brand-surface-highest border border-brand-outline-variant/30">
              <div className="h-full bg-cyan-500/30" style={{ width: `${Math.max(0, Math.min(100, getLeftPct(Date.now())))}%` }} />
            </div>

            {/* Background Events (Gantt Chart Style) */}
            {showBackgroundEvents && BACKGROUND_EVENTS.map((evt, i) => {
              const evtStart = Math.max(evt.start, bounds.start);
              const evtEnd = Math.min(evt.end, bounds.end);
              if (evtStart >= bounds.end || evtEnd <= bounds.start) return null;
            ...
            {showBaseDate && uniqueBaseDates.map(({ time }) => {
              const now = Date.now();
              ...
              if (time < now && leftPct < 100 && todayLeftPct > 0) {
                ...
                if (renderWidth > 0) {
                  const diffYears = ((now - time) / (1000 * 60 * 60 * 24 * 365.25)).toFixed(1);
                  return (
                    <div 
                      key={`diff-box-${time}`}
              ...
              if (leftPct >= 0 && leftPct <= 100) {
                ...
                return (
                  <div 
                    ...
                    style={{ left: `${leftPct}%`, top: '50%', height: '70px' }}
                  >
            ...
            {showBaseDate && uniqueBaseDates.map(({ time, dateStr }) => {
              const leftPct = getLeftPct(time);
              if (leftPct < 0 || leftPct > 100) return null;
              
              const isEditing = editingBaseTime === time;
              const targetLabel = columns.find(c => c.id === targetColId)?.label || '';
              const defaultLabel = `견적일 ${targetLabel} 기준\n(${dateStr})`;
              const currentLabel = currentConfig.baseDateLabels?.[time] || defaultLabel;
                    ...
                    <div className="flex items-center justify-center bg-brand-surface/90 backdrop-blur-sm border border-amber-500/30 px-3 py-1.5 rounded-md shadow-lg group-hover:border-amber-500/60 transition-colors min-w-max">
                      {isEditing ? (
                        <textarea
                          ...
                          rows={2}
                          style={{ width: `${Math.max(120, editValue.length * 6)}px` }}
                        />
                      ) : (
                        <span 
            ...
            {timelineItems.map((item, idx) => {
              ...
              // 4 patterns for vertical staggering to prevent overlap
              const pattern = idx % 4; 
              const isTop = pattern === 0 || pattern === 2;
              const isFar = pattern === 0 || pattern === 1;
              const yOffset = isFar ? '55px' : '20px';

              const displayTitle = item.extraLabel ? `${item.title} (${item.extraLabel})` : item.title;
              ...
              return (
                <div 
                  ...
                  style={{ left: `${leftPct}%` }}
                  onClick={() => handleItemClick(item.reqTitle)}
                >
                  <div className="w-3 h-3 rounded-full bg-cyan-500 ring-2 ring-brand-bg transition-transform group-hover:scale-125 relative z-10" />
                  
                  <div 
                    className="absolute w-px bg-brand-outline-variant/50"
                    style={{ 
                      height: yOffset,
                  ...
                  <div 
                    ...
                    title={tooltipText}
                  >
                    <div className="flex flex-col items-center justify-center bg-brand-surface/95 border border-brand-outline-variant/60 px-2 py-1 rounded shadow-md group-hover:border-cyan-500/50 transition-colors">
                      <span className="text-xs font-bold text-brand-on-surface whitespace-nowrap">{displayTitle}</span>
                      {item.diffMonths !== null && (
                        <span className="text-[11px] font-bold text-rose-400 mt-0.5 whitespace-nowrap">{formatDiff(item.diffMonths)}</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="px-4 pb-3 flex items-center gap-4 text-[10px] text-slate-500">
        <div className="flex items-center gap-1">
          ...
          <span>{columns.find(c => c.id === targetColId)?.label || '기준일'}</span>
        </div>
        {showBaseDate && (
          <div className="flex items-center gap-1">
            <div className="w-2 h-2 rounded-full bg-amber-500 opacity-70" />
            <span>{columns.find(c => c.id === baseColId)?.label || '본함 견적일'}</span>
          </div>
        )}
