...
import { Save, FolderSearch, Globe, FileSpreadsheet, Server, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function SettingsPage({ onConfigSaved }: SettingsPageProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveMessage({ type: 'success', text: '설정이 성공적으로 저장되었습니다. 데스크톱 앱일 경우 부분 재시작이 필요할 수 있습니다.' });
      onConfigSaved();
    } catch (e: any) {
      setSaveMessage({ type: 'error', text: e.message || '저장 중 오류가 발생했습니다.' });
    } finally {
      setIsSaving(false);
    }
  };

  const handleBrowse = async (field: 'activeDataPath' | 'excelExportPath', isDirectory: boolean = false) => {
    ...
    if (window.__TAURI_INTERNALS__) {
      try {
        ...
        const selected = await open({
          ...
          multiple: false,
          defaultPath,
          filters: !isDirectory ? [{ name: 'JSON Data', extensions: ['json'] }] : undefined
        });
      ...
      alert('데스크톱 앱(Tauri) 환경에서만 로컬 파일 파일 탐색기를 사용할 수 있습니다. 웹 브라우저 환경에서는 직접 경로를 입력해주세요.');
    }
  };

  if (isLoading) {
    return <div className="p-8 text-brand-on-surface-variant text-sm">설정 불러오는 중...</div>;
  }

  return (
    <div className="flex-1 overflow-auto bg-brand-surface-lowest">
      <div className="p-8 max-w-4xl mx-auto animate-fade-in">
        <h2 className="text-2xl font-bold text-brand-on-surface mb-2 font-title-md">환경 설정</h2>
        <p className="text-sm text-brand-on-surface-variant mb-8">
          다중 사용자 동기화 데이터 경로, 서버 주소, 내보내기 위치 등을 설정할 수 있습니다.
        </p>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-brand-surface border border-brand-outline-variant p-5 rounded-xl">
            ...
            <p className="text-xs text-brand-on-surface-variant mb-3">
              요구조건 목록과 담당자 풀을 동기화할 메인 JSON 파일의 공용 경로를 지정합니다. 다중 사용자가 같은 파일을 바라보도록 설정하세요.
            </p>
            ...
            {config.cwd && (
              <p className="text-[10px] text-brand-on-surface-variant mt-2">
                기본 실행 경로: <code className="bg-brand-surface-lowest px-1 rounded">{config.cwd}</code>
              </p>
            )}
          </div>
          ...
          <div className="bg-brand-surface border border-brand-outline-variant p-5 rounded-xl">
            ...
            <p className="text-xs text-brand-on-surface-variant mb-3">
              시트를 Excel 파일로 내보낼 때 기본적으로 저장될 폴더 경로를 지정합니다.
            </p>
            <div className="flex items-center gap-2">
              <input 
                type="text" 
                value={config.excelExportPath}
                onChange={e => handleChange('excelExportPath', e.target.value)}
          ...
          <div className="bg-brand-surface border border-brand-outline-variant p-5 rounded-xl">
            ...
            <p className="text-xs text-brand-on-surface-variant mb-4">
              빈 공유 폴더에 이 데스크톱 앱을 복사해 두었다면, 이 버튼을 통해 데이터베이스 거점(locks, changelogs, data 폴더 등)을 일괄 생성하여 실시간 충돌 방지 시스템을 초기화할 수 있습니다.
            </p>
            <button 
              onClick={async () => {
                // @ts-ignore
                if (window.__TAURI_INTERNALS__) {
                  ...
                  const projectName = prompt("프로젝트 명칭을 입력하세요\n(입력된 명칭에 따라 data 저장소가 '{명칭}.json'으로 자동 설정됩니다.)", `프로젝트_${todayStr}`);
                  if (!projectName) {
                    return; // cancelled
                  }
                  ...
                  try {
                    ...
                    const res: any = await invoke('admin_setup_server_environment', { projectName });
                    ...
                    setSaveMessage({ type: 'success', text: `[${projectName}] 인프라가 성공적으로 스캐폴딩 되었습니다.\n생성된 경로들을 동기화하려면 [설정 저장] 버튼을 누르세요.` });
                  } catch (e: any) {
                    console.error('Setup failed', e);
                    setSaveMessage({ type: 'error', text: `설치 중 오류가 발생했습니다: ${e.message || e}` });
                  }
                } else {
                  alert('데스크톱 앱(Tauri) 환경에서만 사용 가능한 관리자 시스템입니다.');
                }
              }}
          ...
          {/* Sample Data Reset Tool */}
          <div className="bg-red-500/5 border border-red-500/20 p-5 rounded-xl">
            <h3 className="text-sm font-semibold flex items-center gap-2 mb-4 text-red-500">
              <RefreshCw className="w-4 h-4" />
              샘플 데이터 초기화 (물가상승 현가화 예제 복원)
            </h3>
            <p className="text-xs text-brand-on-surface-variant mb-4">
              모든 로컬 데이터를 지우고 초기 샘플 데이터로 복구합니다.
            </p>
            <button 
              onClick={() => {
                if (confirm('현재 저장된 모든 데이터가 삭제되고 초기 샘플 데이터로 복원됩니다. 계속하시겠습니까?')) {
                  localStorage.removeItem('offline_db');
                  localStorage.removeItem('app_config');
                  window.location.reload();
                }
              }}
              className="bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/30 px-4 py-2.5 rounded-lg font-medium text-sm transition-colors cursor-pointer w-full text-center"
            >
              🔄 데이터 초기화 및 재시작
            </button>
          </div>
        ...
        <div className="mt-8 flex items-center gap-4">
          ...
          {saveMessage && (
            <div className={`text-sm flex items-center gap-2 ${saveMessage.type === 'success' ? 'text-brand-success' : 'text-brand-error'}`}>
