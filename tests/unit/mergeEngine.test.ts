...
// ─── 결함 #9 회귀: 부분 페이로드가 대량 삭제로 오해석되지 않아야 함 ───
it("[결함#9 수정] theirs에 tabs 키가 없어도 탭이 삭제되지 않는다(부분 페이로드 방어)", () => {
  const base = snap([req("REQ-001")]);
  const mine = snap([req("REQ-001")]);
  // 외부 도구/스크립트의 부분 저장: tabDataMap만 있고 tabs 키 자체가 없음
  const theirs: any = { tabDataMap: { T1: { requirements: [req("REQ-001")], columns: [] } } };

  const r = computeSmartMerge(base, mine, theirs);
  assert.equal(
    (r.mergedPayload.tabs || []).length,
    1,
    "tabs 키 부재는 '전체 탭 삭제'가 아니라 '변경 없음'으로 간주되어야 함",
  );
  assert.equal(r.mergedPayload.tabs![0].id, "T1");
});

it("[결함#9 수정] theirs에 tabDataMap의 특정 탭 엔트리가 없어도 그 탭의 요구항목이 삭제되지 않는다", () => {
  const base = snap([req("REQ-001"), req("REQ-002")]);
  const mine = snap([req("REQ-001"), req("REQ-002")]);
  // theirs는 다른 탭(T2)만 담은 부분 페이로드 — T1 엔트리 부재
  const theirs: any = {
    tabDataMap: { T2: { requirements: [req("REQ-100")], columns: [] } },
    tabs: [{ id: "T1", name: "탭1" }, { id: "T2", name: "탭2" }],
  };
  ...
  assert.deepEqual(
    ids(merged(r)),
    ["REQ-001", "REQ-002"],
    "T1 엔트리 부재는 'T1 전체 요구항목 삭제'가 아니라 '변경 없음'으로 간주되어야 함",
  );
});

it("[결함#9 인접] 정상 페이로드의 탭 삭제(tabs 배열에서 제외)는 여전히 반영된다", () => {
  const base: any = {
    ...snap([req("REQ-001")]),
    tabs: [{ id: "T1", name: "탭1" }, { id: "T2", name: "탭2" }],
  };
  const mine: any = {
    ...snap([req("REQ-001")]),
    tabs: [{ id: "T1", name: "탭1" }], // 내가 T2 삭제 (tabs 배열은 존재)
  };
  const theirs: any = {
    ...snap([req("REQ-001")]),
    tabs: [{ id: "T1", name: "탭1" }, { id: "T2", name: "탭2" }],
  ...
  const r = computeSmartMerge(base, mine, theirs);
  assert.deepEqual(
    (r.mergedPayload.tabs || []).map((t: any) => t.id),
    ["T1"],
    "배열이 존재하는 정상 페이로드의 탭 삭제 의미는 유지되어야 함(회귀 방지)",
  );
});
